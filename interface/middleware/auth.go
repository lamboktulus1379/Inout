...
func Auth(userRepository repository.IUser, userActivityRepository repository.IUserActivity) gin.HandlerFunc {
	...
	return func(ctx *gin.Context) {
		...
		if token.Valid {
			...
			user, err := userRepository.GetByUserName(ctx.Request.Context(), userClaims.UserName)
			if err != nil {
				fmt.Println("User not found")
				ctx.AbortWithStatusJSON(http.StatusUnauthorized, res)
				return
			}
			lastLogin, err := userActivityRepository.GetLastLogin(ctx, user.ID)
			if err != nil {
				fmt.Println("User not found")
				ctx.AbortWithStatusJSON(http.StatusUnauthorized, res)
				return
			}
			lastLogout, err := userActivityRepository.GetLastLogout(ctx, user.ID)
			if err != nil {
				fmt.Println("User not found")
				ctx.AbortWithStatusJSON(http.StatusUnauthorized, res)
				return
			}
			a := lastLogin.CreatedAt
			b := lastLogout.CreatedAt
			fmt.Println(a, b)
			if a.Before(*b) {
				res.ResponseMessage = "User has been logout"
				ctx.AbortWithStatusJSON(http.StatusUnauthorized, res)
				return
			}
